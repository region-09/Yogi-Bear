...
public class Game {
    ...
    private List<List<Integer>> rangers;
    int dir = 1;
    public int time = 0;
    ...
    public void loadGame(GameID gameID) {
        ...
        for (int i = 0; i < gameLevel.level.length; i++) {
            for (int j = 0; j < gameLevel.level[i].length; j++) {
                if (gameLevel.level[i][j] == LevelItem.RANGERV) {
                ...
                if (gameLevel.level[i][j] == LevelItem.RANGER) {
                    List<Integer> ls = new ArrayList<>();
                    dir = rand.nextInt(2) == 0 ? 3 : 2;
                    ls.add(dir);
                    ls.add(i);
                    ls.add(j);
                    rangers.add(ls);
    ...
    // This method checks if player caught by the ranget or not
    public boolean check() {
        if (gameLevel.isGameEnded() == false) {
            int j = gameLevel.player.x;
            int i = gameLevel.player.y;
            if ((LevelItem.RANGER == gameLevel.level[i + 1][j] || LevelItem.RANGERV == gameLevel.level[i + 1][j]) ||
                (LevelItem.RANGER == gameLevel.level[i - 1][j] || LevelItem.RANGERV == gameLevel.level[i - 1][j]) ||
                (LevelItem.RANGER == gameLevel.level[i][j + 1] || LevelItem.RANGERV == gameLevel.level[i][j + 1]) || 
                (LevelItem.RANGER == gameLevel.level[i][j - 1] || LevelItem.RANGERV == gameLevel.level[i][j - 1]) ||
                (LevelItem.RANGER == gameLevel.level[i - 1][j - 1] || LevelItem.RANGERV == gameLevel.level[i - 1][j - 1]) ||
                (LevelItem.RANGER == gameLevel.level[i + 1][j - 1] || LevelItem.RANGERV == gameLevel.level[i + 1][j - 1]) ||
                (LevelItem.RANGER == gameLevel.level[i - 1][j + 1] || LevelItem.RANGERV == gameLevel.level[i - 1][j + 1])  ||
                (LevelItem.RANGER == gameLevel.level[i + 1][j + 1] || LevelItem.RANGERV == gameLevel.level[i + 1][j + 1]) ) {
               numLive--;
    ...
    // This method is responsible for moving player from one position to another
    // Also it will update previous or changed tile.
    public void move() {
        for (int i = 0; i < rangers.size(); i++) {
            List<Integer> entry = rangers.get(i);
            int direction = entry.get(0);
            int y = entry.get(1);
            int x = entry.get(2);
            if (direction == 0) {
                if (gameLevel.level[y - 1][x] == LevelItem.EMPTY) {
                    gameLevel.level[y - 1][x] = LevelItem.RANGERV;
                    gameLevel.level[y][x] = LevelItem.EMPTY;
                    entry.set(1, y - 1);
                }else {
                    entry.set(0, 1);
                }
                ...
                if (gameLevel.level[y][x + 1] == LevelItem.EMPTY) {
                    gameLevel.level[y][x + 1] = LevelItem.RANGER;
                    gameLevel.level[y][x] = LevelItem.EMPTY;
                    entry.set(2, x + 1);
                }else {
                    entry.set(0, 3);
    ...
    public void setName(String input) {
        if (gameLevel.gameID.name.equals("none")) {
            gameLevel.gameID.setName(input);
        }
        database.storeHighScore(gameLevel.gameID, gameLevel.getNumSteps());
    }
    ...
    // helper function for reading line
    ...
    // Helper function for reading from file. taking certain levels name and number
